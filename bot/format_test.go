...
func TestОдиночнаяЦельДаётТуЖеКарточку(t *testing.T) {
	// РАНЬШЕ ЗДЕСЬ ПРОВЕРЯЛОСЬ ОБРАТНОЕ: прогон из одной цели обязан был дать
	// «прежнюю форму» — «X обновлён», версия отдельной строкой, без следов
	// группировки. От этого требования отказались сознательно.
	//
	// Одна и та же выкатка получала две разные вёрстки, и выбирало между ними
	// число целей в прогоне — обстоятельство, к сути релиза отношения не
	// имеющее. В ленте это читалось как отсутствие единого дизайна: у одного
	// сообщения шапка «X обновлён», у соседнего «X · sha»; версия то во второй
	// строке, то в строке цели; время то с подписью «собрано», то без.
	//
	// Теперь вёрстка одна на все релизы, и этот тест сторожит именно её.
	...
	if !strings.Contains(head, "Snakes") {
		t.Errorf("в шапке нет имени проекта:\n%s", head)
	}
	for _, want := range []string{
		"выкачен",
		d.Version,
		"была <code>" + d.Previous + "</code>",
		"<b>Изменения</b>",
	} {
		if !strings.Contains(got, want) {
